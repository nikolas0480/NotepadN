...
Особенностью архитектуры является использование `tree-sitter` для подсветки синтаксиса. Он работает в Rust-бэкенде: парсит текст и отправляет токены подсветки на фронтенд.
...
- **Парсер синтаксиса**: tree-sitter (на стороне Rust)
